...
export function HomeView({ setCurrentTrack, setIsPlaying }: HomeViewProps) {
    ...
    <div className="p-8">
          ...
          {recentlyPlayed.map((item) => (
            ...
            >
              <div className="flex items-center p-4">
                <div className="w-16 h-16 bg-gradient-to-br from-purple-600 to-blue-600 rounded flex items-center justify-center text-2xl mr-4">
                  {item.image}
                </div>
                <div className="flex-1">
                  <h3 className="text-white font-semibold">{item.title}</h3>
                  <p className="text-spotify-light-gray text-sm capitalize">{item.type}</p>
                </div>
                <Button
                  size="sm"
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-5 gap-4">
          {madeForYou.map((item) => (
            <Card
              key={item.id}
              className="bg-spotify-dark-gray hover:bg-spotify-gray transition-all duration-200 cursor-pointer group p-4"
            >
              <div className="relative mb-4">
                <div className="w-full aspect-square bg-gradient-to-br from-green-600 to-blue-600 rounded-lg flex items-center justify-center text-4xl">
                  {item.image}
                </div>
                <Button
                  size="sm"
                  className="absolute bottom-2 right-2 bg-spotify-green hover:bg-spotify-green-hover text-black rounded-full w-12 h-12 opacity-0 group-hover:opacity-100 transition-opacity duration-200"
                  onClick={() => handlePlayTrack(item)}
                >
                  <Play className="w-5 h-5 fill-current" />
                </Button>
              ...
              <p className="text-spotify-light-gray text-sm line-clamp-2">{item.subtitle}</p>
            </Card>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="bg-spotify-dark-gray rounded-lg p-4">
          <div className="grid grid-cols-[16px_1fr_1fr_1fr_60px] gap-4 text-spotify-light-gray text-sm font-medium mb-4 px-4">
            <div>#</div>
            <div>TITLE</div>
            <div>ALBUM</div>
            <div>DATE ADDED</div>
          ...
          <div className="space-y-2">
            {popularTracks.map((track, index) => (
              <div
                key={track.id}
                className="grid grid-cols-[16px_1fr_1fr_1fr_60px] gap-4 items-center p-2 rounded hover:bg-spotify-gray group cursor-pointer"
                onClick={() => handlePlayTrack(track)}
              >
                <div className="text-spotify-light-gray text-sm">{index + 1}</div>
                <div>
                  <div className="text-white font-medium">{track.title}</div>
                  <div className="text-spotify-light-gray text-sm">{track.artist}</div>
                </div>
                <div className="text-spotify-light-gray text-sm">{track.album}</div>
                <div className="text-spotify-light-gray text-sm">3 days ago</div>
                <div className="text-spotify-light-gray text-sm">{track.duration}</div>
              </div>
            ))}
          </div>
        </div>
